...
- Improve types definitions where `?` and `undefined` are redundant by removing `undefined`:
...
- Enforce types definitions for optional properties of the (voluntarily undocumented and) unexposed config functions `setConfig` and `setTzConfig`: it was possible to use them by omitting the `separator` property, it’s not anymore.
